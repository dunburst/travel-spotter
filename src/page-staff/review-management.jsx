...
export default function ReviewManagementPage() {
    ...
    const [activeTab, setActiveTab] = useState('pending'); // State cho tab
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Sắp xếp dữ liệu mới nhất lên trước
    ...
    const handleApprove = async (reviewId) => {
        ...
        try { await approveReview(reviewId); fetchData(); } catch (err) { alert("Lỗi: " + err.message); }
    };
    const handleReject = async (reviewId) => {
        if (!window.confirm("Bạn có chắc muốn từ chối đánh giá này?")) return;
        try { await rejectReview(reviewId); fetchData(); } catch (err) { alert("Lỗi: " + err.message); }
    };
    const handleDelete = async (reviewId) => {
        if (!window.confirm("Hành động này không thể hoàn tác. Bạn có chắc muốn xóa vĩnh viễn đánh giá này?")) return;
        try { await deleteReview(reviewId); fetchData(); } catch (err) { alert("Lỗi: " + err.message); }
    };
